...
def load_and_preprocess_data():
    ...
    # 对model使用LabelEncoder
    if 'model' in train_data.columns:
        print("对model使用LabelEncoder...")
        le = LabelEncoder()
        train_data['model'] = le.fit_transform(train_data['model'].astype(str))
        label_encoders['model'] = le
    
    # 对bodyType使用LabelEncoder
    if 'bodyType' in train_data.columns:
        print("对bodyType使用LabelEncoder...")
        le = LabelEncoder()
        train_data['bodyType'] = le.fit_transform(train_data['bodyType'].astype(str))
        label_encoders['bodyType'] = le
    
    # 对fuelType使用LabelEncoder
    if 'fuelType' in train_data.columns:
        print("对fuelType使用LabelEncoder...")
        le = LabelEncoder()
        train_data['fuelType'] = le.fit_transform(train_data['fuelType'].astype(str))
        label_encoders['fuelType'] = le
    
    # 对regionCode使用LabelEncoder（不进行One-Hot编码）
    if 'regionCode' in train_data.columns:
        print("对regionCode使用LabelEncoder...")
        le = LabelEncoder()
        train_data['regionCode'] = le.fit_transform(train_data['regionCode'].astype(str))
        label_encoders['regionCode'] = le
    
    # 对其他分类特征使用LabelEncoder
    other_categorical_features = [f for f in categorical_features if f not in ['brand']]
...
def prepare_features(data, label_encoders=None, is_training=True):
    ...
    feature_columns = [
        ...
        'notRepairedDamage', 'regionCode', 'model', 'bodyType', 'fuelType'
    ]
    ...
    # model, bodyType, fuelType现在使用LabelEncoder，直接包含在基础特征中
    
    # regionCode现在使用LabelEncoder，直接包含在基础特征中
    ...
    if not is_training and label_encoders:
        ...
        # 对model使用LabelEncoder
        if 'model' in data.columns and 'model' in label_encoders:
            print("对测试数据的model使用LabelEncoder...")
            # 处理训练时未见过的类别
            unique_values = set(data['model'].astype(str).unique())
            known_values = set(label_encoders['model'].classes_)
            new_values = unique_values - known_values
            
            if new_values:
                print(f"发现model中的新类别: {new_values}")
                # 用最频繁的类别替换新类别
                most_common = label_encoders['model'].classes_[0]
                data['model'] = data['model'].astype(str).replace(list(new_values), most_common)
            
            data['model'] = label_encoders['model'].transform(data['model'].astype(str))
        
        # 对bodyType使用LabelEncoder
        if 'bodyType' in data.columns and 'bodyType' in label_encoders:
            print("对测试数据的bodyType使用LabelEncoder...")
            # 处理训练时未见过的类别
            unique_values = set(data['bodyType'].astype(str).unique())
            known_values = set(label_encoders['bodyType'].classes_)
            new_values = unique_values - known_values
            
            if new_values:
                print(f"发现bodyType中的新类别: {new_values}")
                # 用最频繁的类别替换新类别
                most_common = label_encoders['bodyType'].classes_[0]
                data['bodyType'] = data['bodyType'].astype(str).replace(list(new_values), most_common)
            
            data['bodyType'] = label_encoders['bodyType'].transform(data['bodyType'].astype(str))
        
        # 对fuelType使用LabelEncoder
        if 'fuelType' in data.columns and 'fuelType' in label_encoders:
            print("对测试数据的fuelType使用LabelEncoder...")
            # 处理训练时未见过的类别
            unique_values = set(data['fuelType'].astype(str).unique())
            known_values = set(label_encoders['fuelType'].classes_)
            new_values = unique_values - known_values
            
            if new_values:
                print(f"发现fuelType中的新类别: {new_values}")
                # 用最频繁的类别替换新类别
                most_common = label_encoders['fuelType'].classes_[0]
                data['fuelType'] = data['fuelType'].astype(str).replace(list(new_values), most_common)
            
            data['fuelType'] = label_encoders['fuelType'].transform(data['fuelType'].astype(str))
        
        # 对regionCode使用LabelEncoder
        if 'regionCode' in data.columns and 'regionCode' in label_encoders:
            print("对测试数据的regionCode使用LabelEncoder...")
            # 处理训练时未见过的类别
            unique_values = set(data['regionCode'].astype(str).unique())
            known_values = set(label_encoders['regionCode'].classes_)
            new_values = unique_values - known_values
            
            if new_values:
                print(f"发现regionCode中的新类别: {new_values}")
                # 用最频繁的类别替换新类别
                most_common = label_encoders['regionCode'].classes_[0]
                data['regionCode'] = data['regionCode'].astype(str).replace(list(new_values), most_common)
            
            data['regionCode'] = label_encoders['regionCode'].transform(data['regionCode'].astype(str))
...
def build_neural_network(input_dim):
    ...
    model = keras.Sequential([
        ...
        # layers.Dropout(0.01, name='dropout_1'),
        
        # 第2层：全连接层 + BatchNormalization + Dropout
        layers.Dense(128, activation='relu', name='dense_2'),
        # layers.BatchNormalization(name='bn_2'),
        # layers.Dropout(0.01, name='dropout_2'),
...
def train_neural_network():
    ...
    # 编译模型
    model.compile(
        optimizer=keras.optimizers.Adam(learning_rate=0.001),  # 提高学习率
...
def predict_test_set(model, scaler, y_scaler, label_encoders, feature_columns):
    ...
    # 保存结果
    output_file = 'neural_network_price_predictions.csv'
